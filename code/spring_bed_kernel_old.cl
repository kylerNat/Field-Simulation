...
float psi_ddot_edge(int x, int y, float dt, __global float* psi_, __global float* psi_dot_, float psi, float psi_dot, float psi_ddot)
{
    psi_ddot += -(k*4.8+k_center)*psi;
    psi_ddot -= 0.001*2.5*psi_dot;
    return psi_ddot;
                       ...
                       float dt)
{
    ...
    else if(x > 0 && x < w-1 && y > 0 && y < h-1)
    {
        ...
        for(int dx = -1; dx <= 1; dx++)
            ...
            {
                psi_ddot_base += psi_[(x+dx)+(y+dy)*w]*c[1+dx+(1+dy)*3];
            }
